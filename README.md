...
- Authorization callback URL: `https://yyb-decap-oauth.724027324.workers.dev/callback`
...
部署成功后，终端应输出真实 Worker 地址：

```text
https://yyb-decap-oauth.724027324.workers.dev
```
...
backend:
  ...
  branch: main
  base_url: https://yyb-decap-oauth.724027324.workers.dev
  auth_endpoint: /auth
```

回到 GitHub OAuth App，把 Authorization callback URL 设置为：

```text
https://yyb-decap-oauth.724027324.workers.dev/callback
```

如果后台跳到 `api.netlify.com/auth?...site_id=127.0.0.1` 并显示 `Not Found`，说明 Decap CMS 没有使用这个 OAuth Proxy，正在尝试默认 Netlify 认证入口。

部署 OAuth Proxy 后，还需要确认：

- GitHub OAuth App 的 Client ID 和 Client Secret 已配置到 OAuth Proxy。
- GitHub OAuth App 的回调地址指向 `https://yyb-decap-oauth.724027324.workers.dev/callback`。
